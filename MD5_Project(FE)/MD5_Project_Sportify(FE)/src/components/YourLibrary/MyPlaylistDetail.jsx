...
export default function MyPlaylistDetail() {
        ...
        return (
            <tr key={item.id} className='pt-8 hover:bg-[hsla(0,0%,100%,.1)] rounded-md group'>
                ...
                <td className='flex gap-2 items-center py-2'>
                    ...
                    <div><p className='text-compact-1 text-base font-CircularBook text-[#fff]'>{item.name}</p><p className='text-sm'>{item.artist.name}</p></div>
                </td>
    ...
    const elementAudioSearch = audioList?.map((item) => {
        return (
            <tr key={item.id} className='pt-8 hover:bg-[hsla(0,0%,100%,.1)] rounded-md group'>
                <td width={"50%"} className='flex gap-2 items-center py-2 px-2'>
    ...
    return (
        <div>
            ...
            <Navbar />
            {/* Content */}
            <section className='section-playlist pb-[90px] h-full min-h-screen pl-[241px] bg-[#535353]'>
                <div className='section-playlist-banner flex items-end gap-6 linearColor max-h-[500px] h-[30vh] min-h-[340px] p-8'>
                    <div className='banner-img w-48 h-48'>
                        {(selectPlaylist?.image) ? <img className='w-full object-cover h-full drop-shadow-2xl' src={selectPlaylist?.image} alt="" /> :
