        ...
        return (
            <tr key={audio.id} className='pt-8 hover:bg-[hsla(0,0%,100%,.1)] rounded-md group'>
                ...
                <td className='flex gap-2 items-center py-2'>
                    ...
                    <div><p className='text-base font-CircularBook text-[#fff]'>{audio.name}</p><p className='text-sm'>{audio.artist.name}</p></div>
                </td>
                <td className='overflow-hidden truncate'>{audio.album.name}</td>
                <td>
            ...
            <section className='section-playlist pb-[90px] min-h-screen pl-[241px] bg-[#121212] '>
                <div className='section-playlist-banner flex items-end gap-6 linearColor max-h-[500px] h-[30vh] min-h-[340px] p-8'>
                    <div className='banner-img w-48 h-48'>
                        <img className='w-full object-cover h-full drop-shadow-2xl' src={listAudios[0]?.artist?.image} alt="" />
                    </div>
                    <div className='overflow-hidden banner-song-info text-[#fff]'>
                        <p className='text-[16px] font-CircularBook'>{listAudios[0]?.artist?.name}</p>
                        <h3 className='font-CircularBold text-[72px] truncate'>{selectCategory?.name}</h3>
