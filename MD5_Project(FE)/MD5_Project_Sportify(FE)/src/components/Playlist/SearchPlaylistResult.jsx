        ...
        return (
            <tr key={audio.id} className='pt-8 hover:bg-[hsla(0,0%,100%,.1)] rounded-md group'>
                ...
                <td className='flex gap-2 items-center py-2'>
                    ...
                    <div><p className='text-base font-CircularBook text-[#fff]'>{audio.name}</p><p className='text-sm'>{result?.name}</p></div>
                </td>
                <td className='overflow-hidden truncate'>{typeResult === 'artist' ? audio.album.name : audio.name}</td>
                <td>
